...
public class CommHandler extends SimpleChannelInboundHandler<Request> {
	protected static Logger logger = LoggerFactory.getLogger("CommHandler");
	...
	/**
	 * messages pass through this method. We use a black-box design as much as
	...
	public boolean send(GeneratedMessage msg,Channel channel) {
		logger.info("CommHandler - Sending message from commHanndler");
		ChannelFuture cf = channel.writeAndFlush(msg);

		if (cf.isDone() && !cf.isSuccess()) {
			logger.error("CommHandler - Message not sent");
			return false;
		}
		return true;
	}
