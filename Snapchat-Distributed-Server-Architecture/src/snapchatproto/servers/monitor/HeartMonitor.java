 ...
 * under the License.
 */
package snapchatproto.servers.monitor;
...
import snapchatproto.core.Mgmt.Network;
import snapchatproto.core.Mgmt.Network.NetworkAction;*/
...
public class HeartMonitor {
	...
	public boolean startHeartbeat() {
		...
		try {
			...
			System.out.println("I got a node join message");
			n.setFromNodeId(iamNode);
			n.setToNodeId(toNodeId);
			
			RaftHeader.Builder rhb = RaftHeader.newBuilder();
			rhb.setOriginatorNodeId(iamNode);
			rhb.setTime(System.currentTimeMillis());
			
			b.setRaftHeader(rhb.build());
			b.setGraph(n.build());
