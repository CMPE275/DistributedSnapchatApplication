...
import snapchatproto.core.RaftMgmt.Network;
...
public class NetworkManager {
	 ...
	 * @param args
	 */
	public void processRequest(ServerCommunicationMessage mgmt, Channel channel) {
	Network req = mgmt.getGraph();
		if (req == null || channel == null)
			return;
		
		logger.info("Network: node '" + mgmt.getRaftHeader().getOriginatorNodeId() + "' sent a " + mgmt.getGraph().getAction());
		 ...
		 * send a heartbeatMgr
		 */
		if (mgmt.getGraph().getAction() == NetworkAction.NODEJOIN) {
			if (channel.isOpen()) {
				...
				if (socka != null) {
					...
					HeartbeatManager.getInstance().addOutgoingChannel(mgmt.getRaftHeader().getOriginatorNodeId(), isa.getHostName(),
							isa.getPort(), channel, socka);
				}
			} else
				logger.warn(req.getFromNodeId() + " not writable");
		} else if (mgmt.getGraph().getAction() == NetworkAction.NODEJOINACK) {
