...
/*import InboundAppWorker;
import OutboundAppWorker;*/
import snapchatproto.comm.App.NameValueSet;
...
public class DiscreteQueue implements ChannelQueue {
	...
	//protected static AtomicReference<DiscreteQueue> instance = new AtomicReference<DiscreteQueue>();
	...
	public static LinkedBlockingDeque<OneQueueEntry> inbound = new LinkedBlockingDeque<OneQueueEntry>();
	public static LinkedBlockingDeque<OneQueueEntry> outbound = new LinkedBlockingDeque<OneQueueEntry>();
	...
	public static void init() {
		...
		iworker = new InboundAppWorker(tgroup, 1);
		iworker.start();
		oworker = new OutboundAppWorker(tgroup, 1);
		oworker.start();
	...
	public static void enqueueIn(Request req, Channel channel) {
		try {
	...
	public static void enqueueResp(Request reply, Channel channel) {
		if (reply == null)
			return;
